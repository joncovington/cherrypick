...
def test_in_bounds_proposals_are_written_and_loadable(env, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: {
            "ok": True,
            "text": json.dumps(
                {"proposals": [{"param": "stop_trigger_ratio", "value": 0.9, "rationale": "elevated VIX1D"}]}
            ),
        },
    )
...
def test_fenced_json_is_tolerated_garbage_is_not(env, monkeypatch):
    state, cfg = env
    monkeypatch.setattr(
        advise, "_run_claude", lambda *a, **k: {"ok": True, "text": '```json\n{"proposals": []}\n```'}
    )
    out = advise.run(cfg, day=DAY)
...
def test_module_without_bounds_is_skipped(env, monkeypatch):
    ...
    monkeypatch.setattr(
        advise,
        "_run_claude",
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("claude must not be called without bounds")),
    )
    out = advise.run(cfg, day=DAY)
