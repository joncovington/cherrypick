...
def _seed(home: Path) -> None:
    ...
    for i in range(20):
        rows.append({"trade_date": f"2026-07-{i + 1:02d}", "symbol": "SPX",
                     "risk_profile": "control", "net_credit": 2.4, "wing_width": 20,
                     "quantity": 1, "pnl": 500.0, "fees": 6.0, "status": "closed",
                     "exit_time": f"2026-07-{i + 1:02d}T20:10:00",  # meic_ic: closed = exit_time set
                     "ic_order_id": f"old-{i}", "created_at": f"2026-07-{i + 1:02d}T14:31:00"})
    # One in-window session for both sides of the pair.
    for tag, pnl, oid in (("control", -50.0, "new-base"), ("advised:control", 25.0, "new-adv")):
        rows.append({"trade_date": WINDOW_START, "symbol": "SPX", "risk_profile": tag,
                     "net_credit": 2.4, "wing_width": 20, "quantity": 1, "pnl": pnl,
                     "fees": 6.0, "status": "closed", "exit_time": f"{WINDOW_START}T20:10:00",
                     "ic_order_id": oid, "created_at": f"{WINDOW_START}T14:31:00"})
    insert(meic, "ic_trades", rows)


def test_the_base_side_is_windowed_to_the_experiment(tmp_home):
    _seed(tmp_home)
    experiment = {"id": "exp-x", "module": "meic", "base_profile": "control",
                  "params_json": '{"stop_trigger_ratio": 1.1}', "created_session": SESSION,
                  "sessions_run": 1}
...
def test_no_created_session_degrades_to_unwindowed(tmp_home):
    _seed(tmp_home)
    experiment = {"id": "exp-y", "module": "meic", "base_profile": "control",
                  "params_json": "{}", "created_session": None, "sessions_run": 0}
