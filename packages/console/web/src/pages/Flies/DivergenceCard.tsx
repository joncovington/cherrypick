...
export function DivergenceCard({ mode, filter }: { mode: TradingMode; filter: FliesFilter }) {
  const { data, isLoading, dataUpdatedAt } = useQuery<Divergence>({
    queryKey: ["flies-divergence", mode, filter.date, filter.era],
    queryFn: async () => {
      const res = await fetch(`/api/flies/divergence?${fliesQuery(mode, { arm: null, date: filter.date, era: filter.era })}`);
