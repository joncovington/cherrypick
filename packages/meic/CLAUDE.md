...
4. **Market assessment** — split into an account-wide pass (once per iteration) and a per-symbol pass (repeated for each symbol in `symbols`, immediately followed by that symbol's Steps 6–7 before moving to the next symbol — see the per-symbol sub-loop below).
   ...
   **4b. Per symbol (repeat for each symbol in `symbols`, in order):**
   ...
   - **ORB range capture** (if `orb_enabled`): on the first loop iteration at or after 9:35 AM ET, record this symbol's high and low during the first `orb_range_minutes` (5) minutes of trading (9:30–9:35 AM) as `orb_high`/`orb_low` for this symbol. These values persist per symbol for the remainder of the session. If not yet available (pre-9:35 AM), skip ORB evaluation for this symbol this iteration and log the reason (`action: "orb_skip"`, `reason: "pre_range_window"`, tagged with this `symbol`) via `loop_log` so the skip is auditable after the fact rather than simply absent from the log.
