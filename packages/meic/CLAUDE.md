...
| `python src/tt.py get_vix1d` | Live CBOE VIX1D (1-day volatility index) via direct DXLink fetch — not yet wired into any gate | No |
...
| `delta_target` | `0.18` | Target delta for short strikes when IV rank is above `low_iv_credit_floor_iv_rank_max` (0.35). Kept below `max_call_delta_entry_open_volatile`/`max_call_delta_entry_late` (0.19) so the target itself doesn't sit at the hard ceiling — collects more credit than the 0.15/0.16 baseline; ceilings were raised to match (see below) |
| `delta_target_low_iv` | `0.13` | Target delta for short strikes when IV rank is between `min_iv_rank` (0.30) and `low_iv_credit_floor_iv_rank_max` (0.35) — lower/wider than `delta_target`, matching documented MEIC convention of scaling delta down on thinner-IV days rather than holding one fixed target regardless of IV regime |
...
4. **Market assessment** — split into an account-wide pass (once per iteration) and a per-symbol pass (repeated for each symbol in `symbols`, immediately followed by that symbol's Steps 6–7 before moving to the next symbol — see the per-symbol sub-loop below).
   ...
   **4b. Per symbol (repeat for each symbol in `symbols`, in order):**
   ...
   - **Delta target by IV rank**: use `delta_target` (0.18) as the `--short_delta` scan parameter when this symbol's IV rank is above `low_iv_credit_floor_iv_rank_max` (0.35); use `delta_target_low_iv` (0.13) instead when IV rank is between `min_iv_rank` (0.30) and that same threshold. This mirrors documented MEIC convention (16–20 delta specifically at IV rank ≥ 0.50-ish, lower/wider on thinner-IV days) rather than scanning at a single fixed delta regardless of the IV regime — a fixed 0.18 target on a low-IV day was found (this week, live) to just trade one hard-stop failure (OTM distance) for another (credit floor) with no delta value clearing both; adjusting the target itself, not just the OTM-floor thresholds, is the more direct fix. Reuses the same `low_iv_credit_floor_iv_rank_max` boundary that already gates the credit-floor relief, rather than a second independent threshold that could drift out of sync with it.
   ...
   - **VIX1D available, not yet gating anything**: `python src/tt.py get_vix1d` returns the live CBOE 1-day volatility index (verified live 2026-07-06 — streams via a direct DXLink Trade subscription, bypassing the streamer daemon's cache-only HTTP path since VIX1D isn't a traded/managed symbol). This is a more precise intraday-regime signal for 0DTE-specific decisions than the standard 30-day VIX above (which measures a much longer volatility window than a same-day option actually spans), but no threshold/gate has been calibrated for it yet — do not use it to block or force entries until a `regime_vix1d_pause_threshold`-equivalent config key exists and is deliberately added here.
