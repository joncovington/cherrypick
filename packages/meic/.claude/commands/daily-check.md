...
```bash
python -m cherrypick.meic.db get_session_init
```
...
```bash
python -m cherrypick.meic.notify log_event --level INFO --message "Daily connection check: <connected|failed>" --data '{"connected": true|false}'
python -m cherrypick.meic.db set_session_init
```
