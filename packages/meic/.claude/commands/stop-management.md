...
7. **EOD and event force-close** — forced close rules in priority order:
   ...
   - **Physically-settled early force-close** (current time ≥ `physical_settlement_force_close_time` = 15:30 ET): close every open IC whose `symbol` is **not** in `cash_settled_symbols` (QQQ/IWM/etc.). These flatten ~15 min ahead of the general deadline so an ITM short can't slip into assignment during the illiquid, pin-risk-heavy close. Cash-settled ICs are unaffected by this rule and wait for the general force-close below.
   - **General force-close** (current time ≥ `force_close_time` = 15:45 ET): close all remaining open ICs regardless of P&L.
   - Mark as `expired` only if the position is flat and that trade's own `symbol` is in `cash_settled_symbols` (no exchange close needed — cash settlement handles it, but only if fully OTM). If the trade's symbol is not in `cash_settled_symbols`, it must be closed, never left to expire, and a failed close is a **critical, assignment-risk escalation** (immediate marketable-limit retry, `CRITICAL` log if still open) — American-style physically-settled options can be assigned into shares both at expiration and, when a short goes deep ITM, before it. See the assignment-risk escalation in Step 2 of the main loop.
