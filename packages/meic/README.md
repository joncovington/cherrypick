...
- **tastytrade account** (paper or live)
- **SendGrid account** (free tier is sufficient for alert volume)
...
| `sandbox` | `true` for paper trading, `false` for live |
| `email.enabled` | `true` to send alerts via SendGrid |
| `email.from` | Verified SendGrid sender address |
| `email.to` | Address to receive alerts |

### 5. Store your SendGrid API key

The API key is stored in Windows Credential Manager (never in config files or environment variables):
