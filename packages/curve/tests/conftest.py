...
pmcc pattern), so an upstream schema change fails here rather than silently producing empty
snapshots.
"""
...
def managed_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CURVE_DB_PATH", raising=False)
    monkeypatch.delenv("CURVE_CONFIG", raising=False)
    return home
