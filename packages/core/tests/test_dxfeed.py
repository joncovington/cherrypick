...
def test_collects_events_keyed_by_symbol():
    ...
    out = _run(
        dxfeed.collect_events("sess", SENTINEL_CLS, ["A", "B"], 1.0, streamer_factory=_factory(streamer))
    )
    assert set(out) == {"A", "B"}
...
def test_falsy_symbols_are_filtered():
    ...
    out = _run(
        dxfeed.collect_events("s", SENTINEL_CLS, [None, "", "X"], 1.0, streamer_factory=_factory(streamer))
    )
    assert set(out) == {"X"}
...
def test_feed_ends_early_returns_partial():
    ...
    out = _run(
        dxfeed.collect_events(
            "s", SENTINEL_CLS, ["A", "B"], 1.0, streamer_factory=_factory(FakeStreamer([FakeEvent("A")]))
        )
    )
    assert set(out) == {"A"}
...
@pytest.mark.parametrize(
    "value,expected", [("3.5", 3.5), (7, 7.0), (None, None), ("x", None), (float("nan"), float("nan"))]
)
