...
async function main() {
  ...
  var toolFailed = (function(hi){if(!hi||typeof hi!=="object")return false;var tr=hi.tool_response!=null?hi.tool_response:(hi.toolResponse!=null?hi.toolResponse:hi.result);if(tr==null)return false;if(typeof tr==="string")return /\b(error|failed|failure|exception|not found|no such|permission denied|traceback)\b/i.test(tr);if(typeof tr==="object"){if(tr.is_error===true||tr.isError===true||tr.success===false||tr.error!=null)return true;var code=tr.exit_code!=null?tr.exit_code:(tr.exitCode!=null?tr.exitCode:tr.code);if(typeof code==="number"&&code!==0)return true;if(Array.isArray(tr.content)&&tr.is_error===true)return true;}return false;})(hookInput);
