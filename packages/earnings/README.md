...
**Status**: scaffold, with the DoltHub-backed pieces of the scanner implemented and tested live end-to-end. `src/tt.py` and `src/db.py` are still stubs (`NotImplementedError` on live broker calls). `src/scanner.py`'s `get_calendar`, `get_iv_rv`, `get_winrate`, and now `get_candidates` (the full tiered scan) all run against real DoltHub data — verified live 2026-07-06 against that day's actual calendar (EPAC, KRUS, PENG, SAR), producing real, varied per-symbol volume/IV-RV/winrate numbers and correct tiering. Every candidate currently lands in `Reject` because price/term-structure/expected-move/OI/delta/expiration-window (criteria #1–#3, #5–#7) depend on `tt.py`'s still-unimplemented broker calls — this is the one remaining gap, not a bug in the tiering logic itself, and each candidate's `hard_fail_reasons`/`broker_data_error` say so explicitly rather than silently defaulting to a pass. See `CLAUDE.md` for the full operating design.
