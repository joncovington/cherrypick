...
- All three additional criteria (#8, #9, #10) are computable, and `get_candidates` implements the full tiered scan across a day's calendar (`scanner.apply_tiering()` for the pure tier-assignment logic, unit-tested against synthetic Tier 1/2/Near-Miss/Reject cases). **In practice, every candidate lands in Reject in this development environment** — hard filters #1–#3 and #5–#7 (price, term structure, expected move, OI, ATM delta, expiration window, chain completeness) need a live tastytrade session via `tt.py` (a real OAuth2 client, not a stub), which has no credentials configured here; run `python src/tt.py secrets_set` to move past this. Missing/unverifiable criteria show up as `<criterion>_unverified` in `hard_fail_reasons` rather than being silently skipped. Verified live 2026-07-06 against a real day's calendar (4 tickers): volume/IV-RV/winrate came back real and varied per symbol (one ticker landed one step from Tier 1, `near_miss_reasons: ["iv_rv_ratio"]`) while every candidate correctly rejected on the broker-dependent criteria for lack of credentials. A low-`sample_size` winrate (see #9's caveat) is a further reason to treat even a future Tier 1 result with more skepticism than the label alone conveys.
