...
4. **Term structure ≤ -0.004** — `(front_atm_iv - back_atm_iv) / back_atm_iv`, computed via `strategies/iron_fly.py`'s `compute_term_structure()` (iron-fly-specific, not part of the shared `scanner.py` engine — a different strategy might use a different term-structure convention or threshold). This is the core signal: front-month IV must be inflated relative to back-month by a real margin, not just "any negative number," or the trade has no edge over background IV noise.
