...
- All three additional criteria (#8, #9, #10) are computable, and `get_candidates` implements the full tiered scan across a day's calendar (`scanner.apply_tiering()` for the pure tier-assignment logic, unit-tested against synthetic Tier 1/2/Near-Miss/Reject cases). Credentials are now configured, and price/term-structure/expected-move (#1/#4/#6) are real, live tastytrade data — OI, ATM delta, expiration window, and chain completeness (parts of #2/#3/#5/#7) are still `_unverified`; `fetch_price_and_term_structure` doesn't compute them yet. Verified live 2026-07-06 against a real day's calendar (4 tickers): EPAC and PENG both cleared the term-structure filter with real negative readings (-0.64, -0.32), PENG landed one step from Tier 1 (`near_miss_reasons: ["iv_rv_ratio"]`), and every candidate still rejects only on the not-yet-implemented OI/delta/expiration-window checks. A low-`sample_size` winrate (see #9's caveat) is a further reason to treat even a future Tier 1 result with more skepticism than the label alone conveys.
