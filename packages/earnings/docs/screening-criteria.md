...
So **every** row on the scan date whose `when` is blank is admitted and **read as after-the-close**:
...
- **Ordered, not bounded, by the morning forward scan.** This used to also GATE admission — a blank row needed to be in `symbol_watch.covered_symbols` (the morning pass's own liquid-universe snapshot) to be scanned at all, which was meant to keep the raw ~250-row calendar inside the entry scan's ~8s-per-symbol budget. It instead produced its own five-session outage on 2026-08-17..24: the snapshot came back empty, the gate read that as "admit nothing", and 44% of an ordinary night's calendar silently vanished. The cost it existed to control was already bounded by `entry_scan_budget_seconds` regardless of row count, so the gate was redundant with a control that already worked — it is gone (measurement break `entry_calendar_admissibility_gate_removed`, 2026-09-02), and the snapshot now only orders the scan so a budget that does bind still finishes the liquid names first.
