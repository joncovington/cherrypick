...
Both databases' `trades` table is strategy-agnostic (a `strategy` column, `"iron_fly"` today) so a second strategy's paper results land in the same database, distinguishable via `get_pnl_summary --strategy <name>` or the `by_strategy` breakdown in its default output, without a schema change.
...
1. Call `strategies/iron_fly.py get_candidates` for **today's date**, keep only rows with `earnings_timing == "After market close"` (their front-month/reaction-date logic is already correct for an afternoon-today entry).
2. Call `strategies/iron_fly.py get_candidates` for **tomorrow's date**, keep only rows with `earnings_timing == "Before market open"` (a report before tomorrow's open is still ahead of us this afternoon; a same-day BMO report already happened this morning and must not be re-entered).
