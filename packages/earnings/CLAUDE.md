...
- **Unattended paper (automated).** The **cherrypick** orchestrator runs the forced-sampling paper harness
  `cherrypick/earnings/strategy_test_runner.py` (`run_entries` / `run_closes`) on OS-scheduled daily entry (15:45 ET) and
  exit (09:45 ET) tasks it registers and watchdogs — this module has no scheduler of its own. It opens
  the isolated strat_test books (every strategy that clears the screen on every viable name; per-strategy
  by default via `strat_test_portfolio`), always paper-only into
...
- **Forward-preview scan (informational, no trading decision).** `symbol_watch.py refresh` runs on its
  ...
  the entry/exit loop, never touches a ledger. It writes `symbol_watch.json` for scout's read-only
...
| `python -m cherrypick.earnings.strategy_dashboard [--mode live\|paper] [--profile X] [--since YYYY-MM-DD]` | Writes a self-contained, offline HTML dashboard (`cherrypick.core.viz` cards with payloads baked inline, drawn client-side on a plain canvas — no server, no network, no CDN, no charting dependency) — date-axis equity curves with drawdown folded in, regime heat table, rejection histogram, cross-strategy comparison grid. Same numbers as `strategy_report.py` (both read `strategy_metrics.py`). `--profile` defaults to the strat_test family in paper mode. `--mode` (default `paper`) selects the DB and carries a PAPER/LIVE badge; paper writes `reports/strategy_dashboard.html`, live writes `reports/strategy_dashboard_live.html` (separate files, no clobber). |
| `python -m cherrypick.earnings.strategy_section [--mode live\|paper] [--profile X] [--since YYYY-MM-DD] --json` | Emits the compact `cherrypick.core.viz` section payload (KPI tiles + the portfolio equity curve as a date-axis timeseries) the orchestrator's suite dashboard renders as this module's live card (`dashboard.sections`, same pattern as gex/flies). Reads `strategy_metrics.py`, so it can never disagree with the report or dashboard. Mode leads the card subtitle so a live view can never pass for paper. |
