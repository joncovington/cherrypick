...
**Scanner**: candidates are found by this project's own `src/scanner.py`, not an external tool, using the hard filters and tiering defined in [`docs/screening-criteria.md`](docs/screening-criteria.md) — that document is the source of truth for every threshold; do not duplicate or re-derive numbers here. Term structure and expected move (criteria #4/#6) are computed live from tastytrade option chains via `tt.py get_option_chain --include_greeks`. IV/RV ratio (#10) is implemented via `scanner.fetch_iv_rv_ratio()` against DoltHub's `post-no-preference/options.volatility_history`. Winrate (#9) requires a historical realized-move backtest **not yet implemented** — per the screening doc, this caps every candidate at Tier 2 at best today; treat "Tier 1" as currently unreachable rather than relaxing the bar to compensate. The earnings-calendar source is DoltHub's `post-no-preference/earnings` dataset (`earnings_calendar_source: "dolthub"` in config); both it and the options dataset are served by one locally-running `dolt sql-server --data-dir <parent>` covering both clones as separate databases on the same port — free, no rate limit, no API key. Verified live 2026-07-06 end-to-end (real clone, real server, real query results for both). Finnhub's free tier is a documented fallback but not wired up.
...
| `python src/scanner.py get_candidates --date MM/DD/YYYY` | Apply `docs/screening-criteria.md`'s layer-1 filters/tiering to that date's calendar | No |
...
4. **Entry window** (if in entry window):
   ...
   **4b. Per candidate (Tier 1 only per `docs/screening-criteria.md`; see that doc's note on Tier 1 being currently unreachable until Tier B is implemented — in the interim, no candidate is eligible for automatic entry, only logging):**
