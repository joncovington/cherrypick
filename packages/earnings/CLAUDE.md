...
| `python -m cherrypick.earnings.db get_open_positions` / `save_trade` / `save_close` / `get_open_legs` / `save_leg_close` / `log_scan` | Persistence (real trades) |
| `python -m cherrypick.earnings.db_paper` (same cmds) | Persistence (paper trades) |
...
| `symbol_screen` | Per-criterion strictness for the five soft screening criteria (`avg_volume`, `winrate`, `iv_rv_ratio`, `market_cap`, `combined_option_volume`) — each set to `"pass"` (strict `min_*`), `"near_miss"` (looser `near_miss_min_*`), or `"off"`. Hard filters always apply. See `docs/screening-criteria.md` / `docs/03-configuration.md`. |
