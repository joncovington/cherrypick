...
def test_evaluate_position_profit_target():
    ...
    quotes = {
        "FC": {"bid": 0.02, "ask": 0.05},
        "FP": {"bid": 0.02, "ask": 0.05},
        "BC": {"bid": 3.0, "ask": 3.1},
        "BP": {"bid": 3.0, "ask": 3.1},
    }
    result = double_calendar.evaluate_position(
        position, _open_legs(), quotes, config={"profit_target_pct": 0.10}
    )
...
def test_evaluate_position_time_exit_near_expiration():
    ...
    result = double_calendar.evaluate_position(
        position, _open_legs(), quotes={}, config={"exit_days_before_front_expiration": 5}
    )
...
def test_evaluate_position_leg_stop_on_high_delta():
    ...
    result = double_calendar.evaluate_position(
        position, _open_legs(), quotes, config={"leg_stop_delta_abs": 0.45}
    )
...
def test_evaluate_position_overnight_gap_suffix():
    ...
    quotes = {"FC": {"delta": 0.55}}
    result = double_calendar.evaluate_position(
        position, _open_legs(), quotes, config={"leg_stop_delta_abs": 0.45}, is_first_check_of_day=True
    )
