...
def _vix_level(readings: dict[str, Any], history: dict[str, list[dict]]) -> dict:
    ...
    if len(closes) < MIN_HISTORY_FOR_RANK:
        return _signal(
            "vix_level", label, UNKNOWN, f"only {len(closes)} of {MIN_HISTORY_FOR_RANK} history sessions held"
        )
    ...
    return _signal(
        ...
        label,
        MEASURED,
        f"VIX {vix:.2f} at the {rank:.0f}th percentile of {len(closes)} sessions" + adjustment,
        score=_clamp(score),
        value=vix,
    )
...
def _term_structure(readings: dict[str, Any]) -> dict:
    ...
    return _signal(
        ...
        score=_linear(ratio, TERM_STRESS_RATIO, TERM_CALM_RATIO),
        value=round(ratio, 4),
    )
...
def _breadth(history: dict[str, list[dict]], sector_symbols) -> dict:
    ...
    return _signal(
        ...
        score=_linear(pct, BREADTH_FLOOR_PCT, BREADTH_CEIL_PCT),
        value=round(pct, 1),
    )
...
def _credit(history: dict[str, list[dict]]) -> dict:
    ...
    return _signal(
        ...
        score=_linear(z, CREDIT_STRESS_Z, CREDIT_CALM_Z),
        value=round(z, 2),
    )
...
def _vix_roc(readings: dict[str, Any], history: dict[str, list[dict]]) -> dict:
    ...
    if len(closes) < ROC_SESSIONS + 1 or closes[-(ROC_SESSIONS + 1)] == 0:
        return _signal("vix_roc", label, UNKNOWN, f"fewer than {ROC_SESSIONS + 1} VIX history sessions held")
    ...
    return _signal(
        ...
        score=_linear(roc, ROC_STRESS_PCT, ROC_CALM_PCT),
        value=round(roc, 1),
    )
