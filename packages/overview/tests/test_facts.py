...
The fixtures deliberately leave ``day_close`` NULL everywhere -- the production producer stops at
the bell, so the settled close only ever appears as ``prev_day_close`` on the next session's row.
A fixture with day_close set would test a cache that does not exist.
"""
...
def _make_cache(rows_summary=(), rows_trades=()):
    ...
    conn.executemany(
        "INSERT INTO stream_summary (symbol, trade_date, prev_day_close, updated_at) "
        "VALUES (?, ?, ?, ?)", rows_summary)
    conn.executemany(
        "INSERT INTO stream_trades (symbol, last, updated_at) VALUES (?, ?, ?)", rows_trades)
    conn.commit()
    conn.close()
...
def test_todays_summary_row_yields_the_prior_settle():
    ...
    _make_cache(rows_summary=[("SPX", SESSION, 7798.99, NOW_TS - 600),
...
def test_without_todays_row_the_last_trade_is_the_prior_value():
    ...
    _make_cache(rows_summary=[("SPX", PRIOR, 7744.90, NOW_TS - 60000)],
...
def test_vix_falls_back_to_meic_market_context():
    ...
    conn.execute("CREATE TABLE market_context (context_date TEXT, vix REAL, vix1d REAL, "
                 "vix1d_ratio REAL, symbols_json TEXT, updated_at REAL)")
    conn.execute("INSERT INTO market_context (context_date, vix, updated_at) VALUES (?, ?, ?)",
...
def test_gex_levels_and_sector_board_flow_into_gates():
    summaries = [("SPX", SESSION, 7798.99, NOW_TS - 600),
                 ("SPX", PRIOR, 7744.90, NOW_TS - 60000),
