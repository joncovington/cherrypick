...
function _spotHistoryPlugin(history,labels,openTs,closeTs){
  return { id:'spotHistory', afterDatasetsDraw(chart){
    ...
    ctx.fillStyle='#7ec8f2'; ctx.beginPath(); ctx.arc(lx,ly,2.5,0,Math.PI*2); ctx.fill();
    ctx.restore();
  }};
}
...
function renderGexMainChart(series,spot,zero,mode,callWall,putWall,spotHistory,marketOpenTs,marketCloseTs){
  ...
  if(spot!=null) series=series.filter(s=>Math.abs(s.strike-spot)<=120);  // gexbot-like span: enough strikes to read as a dense stack
  const labels=series.map(s=>s.strike);
  let ds,titleText;
  if(mode==='oivol'){
    ds=[
...
def make_handler(cfg: dict, default_sym: str):
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def _send(self, code: int, body: bytes, ctype: str) -> None:
            ...
            # holding a cached copy shows a stale layout after a restart until a hard refresh.
