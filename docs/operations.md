...
| `lossdog-notify` | every 10 min — opt-in | `run.py notify-lossdog` | private-API poll (Lossdog VIP feed) with a per-run minted token → its own job, never on the watchdog tick; a dead credential warns once and goes quiet until it changes; an unreadable OS keyring is an outage, not a missing secret, and only warns after an hour of refusals |
