...
| `status-digest` | every **60 min**, 10:00–16:10, trading days | `run.py notify-status` | hourly suite-status card to Discord (off by default, `status_digest.enabled`); a webhook push, so its own job and never a watchdog-tick call |
| `status-digest-close` | daily **16:35**, trading days | `run.py notify-status --close` | the day's CLOSE card: same composition after the 0DTE books settle and the 16:30 review-provisional build, deltas against the last hourly post |
...
| `bwb-live` | every 60 s — **enabled only while the arm record is valid for today** | `pythonw -m cherrypick.bwb.live_loop --once --live` | armed by `/live-bwb-start` writing `state/bwb-live-arm.json` (fresh YES each day); the same self-disarm and watchdog backstop as flies, through `cherrypick.core.live`. Each tick confirms fills, walks resting orders, settles on an official print when due, makes one entry attempt in the window, and runs the paper trigger/mark pass over the live ledger with the fire seam swapped for order placement. |
