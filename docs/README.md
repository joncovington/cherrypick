...
| Doc | Covers |
|---|---|
| [INSTALL.md](../INSTALL.md) | **Just the commands** — clone to running in four steps, no prose. |
...
- **Calendars** — [`packages/calendars/README.md`](../packages/calendars/README.md) (start here: the trade and the experiment) and [`packages/calendars/CLAUDE.md`](../packages/calendars/CLAUDE.md) (the books, the read-side exit-policy derivation, the honesty rules). Every config key is documented inline in its `config.example.json`.
- **PMCC-99** — [`packages/pmcc/README.md`](../packages/pmcc/README.md) (start here: the trade and the single book) and [`packages/pmcc/CLAUDE.md`](../packages/pmcc/CLAUDE.md) (the experiment design, the honesty rules — early assignment is measured, never modelled — and the deep-window data story). Every config key is documented inline in its `config.example.json`.
- **Curve** — [`packages/curve/README.md`](../packages/curve/README.md) (start here: the roll-yield trade and the three books) and [`packages/curve/CLAUDE.md`](../packages/curve/CLAUDE.md) (the regime read, the honesty rules — assignment and reverse splits are measured, never modelled). Every config key is documented inline in its `config.example.json`.
- **BWB** — [`packages/bwb/README.md`](../packages/bwb/README.md) (start here: the ladder and the four add-on books) and [`packages/bwb/CLAUDE.md`](../packages/bwb/CLAUDE.md) (the experiment design, the trigger-tick substrate, the opt-in call-wall book, and since 2026-09-18 the narrow per-day-armed live path). Every config key is documented inline in its `config.example.json`.
- **Overview** — [`packages/overview/README.md`](../packages/overview/README.md) and [`packages/overview/CLAUDE.md`](../packages/overview/CLAUDE.md) (the fact pack, the five gates, and why missing data can never produce RED). The morning narrative is written outside the package by `scripts/morning_narrative.py`.
...
- **Desk** — ⚠️ **Experimental.** [`packages/desk/README.md`](../packages/desk/README.md) and [`packages/desk/CLAUDE.md`](../packages/desk/CLAUDE.md). The manual trading desk: the suite's only *discretionary* live-order path (meic, earnings, flies and bwb each have a live loop of their own behind their own gate). Its README opens with the warnings; read them in full before using it.
- **Review** — [`packages/review/CLAUDE.md`](../packages/review/CLAUDE.md). The cross-module end-of-day review: one versioned fact set per session over every engine, plus its renders — the suite's one EOD answer since 2026-08-13. Read-only over every other package; the narrative is written outside it by a scheduled agent, so a failed note can never damage the record.
- **Advisor** — [`packages/advisor/README.md`](../packages/advisor/README.md) (start here: what it can and cannot do) and [`packages/advisor/CLAUDE.md`](../packages/advisor/CLAUDE.md) (the operating contract — the AI fence, why paper arms are the dry run, and why an experiment outlives its advice artifact). The deterministic half of the AI advisor; the model itself is invoked by `scripts/advisor_checkpoint.py`, outside every package.
