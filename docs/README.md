...
- **BWB** — [`packages/bwb/README.md`](../packages/bwb/README.md) (start here: the ladder and the four add-on books) and [`packages/bwb/CLAUDE.md`](../packages/bwb/CLAUDE.md) (the experiment design, the trigger-tick substrate, the opt-in call-wall book). Every config key is documented inline in its `config.example.json`.
