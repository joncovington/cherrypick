...
- **PMCC-99** — [`packages/pmcc/README.md`](../packages/pmcc/README.md) (start here: the trade and the three books) and [`packages/pmcc/CLAUDE.md`](../packages/pmcc/CLAUDE.md) (the experiment design, the honesty rules — early assignment is measured, never modelled — and the deep-window data story). Every config key is documented inline in its `config.example.json`.
